...
type Name = string;
type Player = {
    readonly name :Name,    // name 값을 바꾸려할 때 오류
    age ?: Age
}
...
const numbers : readonly number[] = [1,2,3,4];  // readonly 로 불변성을 갖는다.
// numbers.push(1) => push 는 오류. filter, map은 배열을 바꾸지 않기때문에 사용가능

// ------------------------------------------------------

// Tuple : 최소한의 길이를 갖고 특정 위치에 특정 타입이 있어야하는 배열을 생성한다.
const player: readonly [string, number, boolean] = ["nico",1,true]; // 실제 환경에 push 하기 전 오류를 확인할 수 있다. 
// player[0] = "igigi";

// ------------------------------------------------------

let a : undefined = undefined;  // 선택적 타입일 때 undefined 결과를 얻을 수 있다.
let b : null = null;
...
function bye(name : string) : never{     
    // return "xxx"; => return 하지 않아야 하므로 error 발생
    throw new Error("xxx")  // 2번
}

function bye2(name : string | number){     
